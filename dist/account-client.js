...
export async function beginTelegramLogin(phoneInput) {
    const phone = normalizePhone(phoneInput);
    const config = readConfig();
    const client = createClient();
    try {
        await client.connect();
        const result = await client.sendCode({ apiId: config.telegramApiId, apiHash: config.telegramApiHash }, phone);
        return {
...
export async function completeTelegramLoginWithPassword(sessionString, password) {
    const config = readConfig();
    const client = createClient(sessionString);
    try {
        await client.connect();
        await client.signInWithPassword({ apiId: config.telegramApiId, apiHash: config.telegramApiHash }, {
...
function splitTelegramMessage(message) {
    ...
    while (remaining.length > TELEGRAM_TEXT_LIMIT) {
        ...
        if (splitAt < TELEGRAM_TEXT_LIMIT * 0.6)
...
function splitCaptionAndText(message) {
    const trimmed = message.trim();
    if (!trimmed)
        return { caption: "", remaining: "" };
    if (trimmed.length <= TELEGRAM_CAPTION_LIMIT)
        return { caption: trimmed, remaining: "" };
    let splitAt = trimmed.lastIndexOf("\n", TELEGRAM_CAPTION_LIMIT);
    if (splitAt < TELEGRAM_CAPTION_LIMIT * 0.6)
        splitAt = trimmed.lastIndexOf(" ", TELEGRAM_CAPTION_LIMIT);
    if (splitAt < 1)
        splitAt = TELEGRAM_CAPTION_LIMIT;
    return {
        caption: trimmed.slice(0, splitAt).trim(),
        remaining: trimmed.slice(splitAt).trim()
    };
}
function extensionForMime(mimeType, mediaType = "") {
...
function mediaFileFromUrl(mediaUrl, mediaType = "") {
...
async function inputEntityFromUser(client, user) {
    return client.getInputEntity(user);
}
async function resolveExistingPhoneContact(client, phoneInput) {
    const phone = normalizePhone(phoneInput);
    ...
    const user = users.find((item) => item instanceof Api.User);
    if (!user)
        throw new Error("Telegram could not resolve this phone number from existing contacts.");
    return inputEntityFromUser(client, user);
}
async function importPhoneContact(client, input) {
    const phone = normalizePhone(input.recipient);
    const clientId = bigInt(Date.now()).multiply(1000).add(Math.floor(Math.random() * 1000));
    let result;
    try {
        result = await client.invoke(new Api.contacts.ImportContacts({
            contacts: [
    ...
    const user = users.find((item) => item instanceof Api.User && (!userId || item.id.toString() === userId));
    if (!user) {
        throw new Error("Telegram could not resolve this phone number. It may be incorrect, private, or not on Telegram.");
    }
    return inputEntityFromUser(client, user);
}
async function resolveMessagePeer(client, input, allowImport) {
...
export async function sendTelegramMessage(sessionString, input) {
    const recipient = input.recipient.trim();
...
export async function revokeTelegramSession(sessionString) {
    const client = createClient(sessionString);
    try {
        await client.connect();
...
function peerReference(peer) {
    if (peer instanceof Api.User)
        return senderReference(peer);
    if (peer instanceof Api.Channel)
        return [peer.username ? `@${peer.username}` : "", peer.id.toString()].filter(Boolean).join(" ");
...
export async function listenForAccount(sessionString, onIncomingMessage) {
    const client = createClient(sessionString);
    await client.connect();
    await client.getMe();
    client.addEventHandler(async (event) => {
        const message = event.message;
        if (!message || message.out || message.isPrivate === false)
            return;
        try {
...
async function resolveUserMessagePeer(client, recipient) {
...
export async function fetchRecentTelegramMessages(sessionString, limit = 100, recipients = []) {
    ...
    try {
        ...
        for (const target of targets) {
            ...
            for (const item of history) {
                if (!(item instanceof Api.Message))
                    continue;
