...
function readAnswerRules(answerPath) {
    ...
    const rawRules = Array.isArray(parsed)
        ...
        : isObject(parsed) && Array.isArray(parsed.answers)
            ? parsed.answers
            : [];
    return rawRules.flatMap((item) => {
        if (!isObject(item))
            return [];
        const answer = typeof item.answer === "string" ? item.answer.trim() : "";
        const keywords = Array.isArray(item.keywords)
            ? item.keywords.filter((keyword) => typeof keyword === "string").map((keyword) => keyword.trim()).filter(Boolean)
            : [];
        return answer && keywords.length ? [{ answer, keywords }] : [];
    });
}
async function callBotApi(config, method, body, timeoutMs = 15_000) {
...
async function sendReply(config, chatId, text, replyToMessageId) {
...
function findSavedAnswer(rules, question) {
...
async function askResponder(config, question) {
    if (!config.botResponderUrl)
        return "";
    ...
    if (!response.ok)
        throw new Error(`Responder returned HTTP ${response.status}.`);
    const payload = (await response.json().catch(() => null));
    if (!isObject(payload))
        return "";
...
async function answerQuestion(config, state, question) {
    try {
...
async function handleMessage(config, state, message) {
    ...
    if (command === "start") {
        await sendReply(config, chatId, "Hi. Send me a question and I will reply using the answers configured for this bot.", message.message_id);
        return;
    }
    if (command === "help") {
        await sendReply(config, chatId, "Ask a question in plain text. The bot checks saved answers first, then uses the configured responder URL if one is enabled.", message.message_id);
        return;
    }
    if (command === "ping") {
...
async function pollUpdates(config, state) {
    let offset = 0;
    let failures = 0;
    while (!stopping) {
        try {
            ...
            const updates = await callBotApi(config, "getUpdates", requestBody, (config.botPollTimeoutSeconds + 10) * 1000);
            failures = 0;
