...
import { beginTelegramLogin, completeTelegramLoginWithCode, completeTelegramLoginWithPassword, normalizePhone } from "./account-client.js";
...
async function main() {
    ...
    if (!token)
        throw new Error("Usage: npm run login -- --token YOUR_ACCESS_TOKEN");
    ...
    try {
        ...
        const result = codeResult.kind === "password_required"
            ? await completeTelegramLoginWithPassword(codeResult.sessionString, (await rl.question("Telegram 2FA password: ")).trim())
            : codeResult;
