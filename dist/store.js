...
const asIso = (value) => value.toISOString();
export class MultiUserStore {
    pool;
    cipher;
    constructor(databaseUrl, sessionEncryptionKey) {
    ...
    async createUser(displayName) {
        ...
        await this.pool.query("INSERT INTO app_users (id, display_name, token_hash) VALUES ($1, $2, $3)", [id, displayName, hashToken(accessToken)]);
        return { user, accessToken };
    }
    async findOrCreateConfiguredUser(loginId, displayName) {
        const result = await this.pool.query(`INSERT INTO app_users (id, display_name, token_hash, configured_login)
       VALUES ($1, $2, $3, $4)
    ...
    async findUserByAccessToken(accessToken) {
        const result = await this.pool.query("SELECT id, display_name FROM app_users WHERE token_hash = $1", [hashToken(accessToken)]);
        const row = result.rows[0];
        return row ? { id: row.id, displayName: row.display_name } : null;
    }
    async createBrowserSession(accessToken, ttlHours) {
        const user = await this.findUserByAccessToken(accessToken);
        if (!user)
            return null;
        return this.createBrowserSessionForUser(user, ttlHours);
    }
    async createBrowserSessionForUser(user, ttlHours) {
        const id = randomUUID();
        ...
        await this.pool.query("INSERT INTO app_sessions (id, user_id, token_hash, expires_at) VALUES ($1, $2, $3, $4)", [id, user.id, hashToken(sessionToken), expiresAt]);
        return { user, sessionToken, expiresAt: asIso(expiresAt) };
    }
    async findUserByBrowserSession(sessionToken) {
        const result = await this.pool.query(`SELECT users.id, users.display_name
       FROM app_sessions AS sessions
       JOIN app_users AS users ON users.id = sessions.user_id
       WHERE sessions.token_hash = $1 AND sessions.expires_at > NOW()`, [hashToken(sessionToken)]);
        const row = result.rows[0];
        return row ? { id: row.id, displayName: row.display_name } : null;
    }
    async deleteBrowserSession(sessionToken) {
        await this.pool.query("DELETE FROM app_sessions WHERE token_hash = $1", [hashToken(sessionToken)]);
    }
    async createLoginChallenge(userId, phone, phoneCodeHash, sessionString, ttlMinutes) {
        const id = randomUUID();
        const expiresAt = new Date(Date.now() + ttlMinutes * 60_000);
        await this.pool.query("DELETE FROM telegram_login_challenges WHERE expires_at <= NOW() OR user_id = $1", [userId]);
        await this.pool.query(`INSERT INTO telegram_login_challenges
        (id, user_id, phone_ciphertext, phone_code_hash_ciphertext, session_ciphertext, status, expires_at)
       VALUES ($1, $2, $3, $4, $5, 'code_sent', $6)`, [id, userId, this.cipher.encrypt(phone), this.cipher.encrypt(phoneCodeHash), this.cipher.encrypt(sessionString), expiresAt]);
        return { id, expiresAt: asIso(expiresAt) };
    }
    async getLoginChallenge(userId, challengeId) {
        const result = await this.pool.query(`SELECT id, phone_ciphertext, phone_code_hash_ciphertext, session_ciphertext, status, expires_at
       FROM telegram_login_challenges WHERE id = $1 AND user_id = $2`, [challengeId, userId]);
        const row = result.rows[0];
        if (!row)
            return null;
    ...
    async markPasswordRequired(userId, challengeId, sessionString) {
        await this.pool.query(`UPDATE telegram_login_challenges SET status = 'password_required', session_ciphertext = $1
       WHERE id = $2 AND user_id = $3`, [this.cipher.encrypt(sessionString), challengeId, userId]);
    }
    async deleteLoginChallenge(userId, challengeId) {
        await this.pool.query("DELETE FROM telegram_login_challenges WHERE id = $1 AND user_id = $2", [challengeId, userId]);
    }
    async saveTelegramAccount(userId, input) {
        const result = await this.pool.query(`INSERT INTO telegram_accounts
       ...
       RETURNING id, telegram_user_id, display_name, username, session_ciphertext, created_at, updated_at`, [randomUUID(), userId, input.telegramUserId, input.displayName, input.username || null, this.cipher.encrypt(input.sessionString)]);
        const row = result.rows[0];
        if (!row) {
    ...
    async listAccounts(userId) {
        const result = await this.pool.query(`SELECT id, telegram_user_id, display_name, username, session_ciphertext, created_at, updated_at
       FROM telegram_accounts WHERE user_id = $1 ORDER BY created_at ASC`, [userId]);
        return result.rows.map((row) => this.toAccount(row));
    }
    async getAccountWithSession(userId, accountId) {
        const result = await this.pool.query(`SELECT id, telegram_user_id, display_name, username, session_ciphertext, created_at, updated_at
       FROM telegram_accounts WHERE id = $1 AND user_id = $2`, [accountId, userId]);
        return result.rows[0] ? this.toAccountWithSession(result.rows[0]) : null;
    }
    async getAllAccountsWithSessions() {
        const result = await this.pool.query(`SELECT id, telegram_user_id, display_name, username, session_ciphertext, created_at, updated_at
    ...
    async recordMessage(input) {
        const existing = await this.pool.query(`SELECT id, account_id, direction, recipient_ciphertext, text_ciphertext, telegram_message_id, created_at
       FROM telegram_messages
       WHERE account_id = $1 AND direction = $2 AND telegram_message_id = $3
       ORDER BY created_at ASC LIMIT 25`, [input.accountId, input.direction, input.telegramMessageId]);
        const duplicate = existing.rows.find((row) => this.cipher.decrypt(row.recipient_ciphertext) === input.recipient);
        if (duplicate)
            return this.toMessageRecord(duplicate);
        const id = randomUUID();
        ...
        const result = await this.pool.query(`INSERT INTO telegram_messages
    ...
    async listMessages(userId, accountId, limit = 50) {
        const result = await this.pool.query(`SELECT messages.id, messages.account_id, messages.direction, messages.recipient_ciphertext,
       ...
       ORDER BY messages.created_at DESC LIMIT $3`, [accountId, userId, Math.min(Math.max(limit, 1), 100)]);
